...
URL_si <- "https://immobilien.derstandard.at/suche/wien/mieten-wohnung?page=1"
...
anzeigen_PLZ <- as.data.frame(str_extract(anzeigen_Adresse$Adressen, "^\\s*\\d{4}") %>% readr::parse_integer()) |>
  rename ("PLZ" = 1)

anzeigen_details <- page_si |>
